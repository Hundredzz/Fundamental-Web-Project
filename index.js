...
app.get('/add-product', isAuthenticated, authorizeRoles(["Manager", "Staff"]), (req, res) => {
    db.get(`SELECT product_id FROM Products WHERE product_id LIKE 'B%' ORDER BY product_id DESC LIMIT 1`, [], (err, row) => {
        if (err) return res.redirect('/add-product?error=' + encodeURIComponent("เกิดข้อผิดพลาดในการสร้างรหัสสินค้า"));
...
app.post("/add-product-data", isAuthenticated, authorizeRoles(["Manager", "Staff"]), uploader.single('product_image'), (req, res) => {
    ...
    const redirectWithError = (message) => {
        ...
        return res.redirect('/add-product?error=' + encodeURIComponent(message));
    };

    if (!req.file) {
        return redirectWithError("กรุณาอัปโหลดรูปภาพสินค้า");
    }

    db.get(`SELECT product_name FROM Products WHERE product_name = ?`, [name], (checkErr, row) => {
        if (checkErr) return redirectWithError("เกิดข้อผิดพลาดในการตรวจสอบชื่อสินค้า");
        if (row)      return redirectWithError("ชื่อสินค้านี้มีอยู่ในระบบแล้ว");
        ...
        db.run(sql, params, function (err) {
            ...
            if (err.message.includes("UNIQUE constraint failed: Products.product_id")) {
                return redirectWithError("รหัสสินค้านี้มีอยู่ในระบบแล้ว");
            }
            return redirectWithError("เกิดข้อผิดพลาดในการเพิ่มสินค้าใหม่");
        });
    });
});
...
app.get('/edit-product/:id', isAuthenticated, authorizeRoles(["Manager", "Staff"]), (req, res) => {
    ...
    fetch(`http://localhost:${port}/api/product/${req.params.id}`, { headers: { cookie: req.headers.cookie } })
        ...
        .catch(() => res.redirect('/product?error=' + encodeURIComponent("เกิดข้อผิดพลาดในการโหลดข้อมูลสินค้า")));
});
...
app.post("/update-product", isAuthenticated, authorizeRoles(["Manager", "Staff"]), uploader.single('product_image'), (req, res) => {
    ...
    const redirectWithError = (message) => {
        if (req.file) fs.unlink(path.join(__dirname, 'public/img/product_image', req.file.filename), () => {});
        return res.redirect(`/edit-product/${code}?error=` + encodeURIComponent(message));
    };

    if (req.file) {
        const newImagePath = req.file.filename;
        db.get(`SELECT img_path FROM Products WHERE product_id = ?`, [code], (err, row) => {
            if (err) return redirectWithError("เกิดข้อผิดพลาดในการดึงข้อมูลรูปภาพเก่า");
            ...
            db.run(sql, params, function (err) {
                if (err) return redirectWithError("เกิดข้อผิดพลาดในการบันทึกข้อมูลสินค้า");
